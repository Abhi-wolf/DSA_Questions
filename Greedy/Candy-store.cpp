...
candies are provided to you. You are now provided with an attractive offer.
...
So min cost : 1+2 =3.In the second case, you can buy the candy which costs 4 and takes candies
...
Explanation:
For minimimum cost buy the candy withthe cost 1 and get all the other candies for free.
...
candies[], its size N and an integer K as input parameters and returns the minimum amount and maximum amount of money to
buy all candies according to the offer.
...
vector<int> candyStore(int candies[], int N, int K)
{
    ...
    amount.push_back(minAmount);
    amount.push_back(maxAmount);
...
int main()
{
    ...
    vector<int> res = candyStore(candies, N, 2);
    Display(res);
    return 0;
}